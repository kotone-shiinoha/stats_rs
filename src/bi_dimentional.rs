/// functions that takes two argument will be found here
...
pub fn linear_regression(input_x: &Input, input_y: &Input) -> Vec<Num> {
    ...
    input_x
        .iter()
        .map(|i| *i * gradient + intercept).collect::<Vec<Num>>()
}

pub fn exponential_regression(input_x: &Input, input_y: &Input) -> Vec<Num> {
    let mut sum = [0 as Num; 5];
    for i in 0..input_x.len() {
        let x = input_x[i];
        let y = input_y[i];
    ...
    input_x
        .iter()
        .map(|i| a * NumE.log(b * i))
        .collect::<Vec<Num>>()
}

pub fn logarithm_regression(input_x: &Input, input_y: &Input) -> Vec<Num> {
...
pub fn manhattan_distance(input1: &Input, input2: &Input) -> Num {
    distance_base(input1, input2, |dif, dis| {
        dis + dif.abs()
    })
}

pub fn minkowski_distance(input1: &Input, input2: &Input, city_blocks: Num) -> Num {
    let dis = distance_base(input1, input2, |dis, dif| {
        dis + dif.abs().powf(city_blocks)
    });
